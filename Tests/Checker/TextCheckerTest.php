...
class TextCheckerTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$this->assertTrue($this->checker->check('ｱｲｳ'   , array('maxLength' => 9, 'mbLength' => TextChecker::LENGTH_BYTES)));
		$this->assertTrue($this->checker->check('アイウ', array('maxLength' => 9, 'mbLength' => TextChecker::LENGTH_BYTES)));
	}

	public function testCheckIsOkWhenMultiByteCharactersWidth()
	{
		$this->assertTrue($this->checker->check('ｱｲｳ'   , array('maxLength' => 3, 'mbLength' => TextChecker::LENGTH_WIDTH)));
		$this->assertTrue($this->checker->check('アイウ', array('maxLength' => 6, 'mbLength' => TextChecker::LENGTH_WIDTH)));
	}
