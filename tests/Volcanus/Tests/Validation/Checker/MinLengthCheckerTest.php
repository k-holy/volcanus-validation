...
class MinLengthCheckerTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$this->assertTrue($this->checker->check('ｱｲｳ'   , array('length' => 9, 'mbLength' => MinLengthChecker::LENGTH_BYTES)));
		$this->assertTrue($this->checker->check('アイウ', array('length' => 9, 'mbLength' => MinLengthChecker::LENGTH_BYTES)));
	}

	public function testCheckIsOkWhenMultiByteCharactersWidth()
	{
		$this->assertTrue($this->checker->check('ｱｲｳ'   , array('length' => 3, 'mbLength' => MinLengthChecker::LENGTH_WIDTH)));
		$this->assertTrue($this->checker->check('アイウ', array('length' => 6, 'mbLength' => MinLengthChecker::LENGTH_WIDTH)));
	}
